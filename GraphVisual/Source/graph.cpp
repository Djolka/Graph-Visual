...
Graph::Graph(bool directed, bool weighted)
        : m_weightRange(calcWeightRange(weighted)), m_directed(directed),
            m_weighted(weighted) {}
...
bool Graph::isWeighted() const { return m_weighted; }

bool Graph::isUnweighted() const { return !m_weighted; }
...
bool Graph::hasNode(Node *node) const {
    auto iterator = std::find_if(m_nodes.begin(), m_nodes.end(), [node](Node *x) {
...
bool Graph::changeNodeName(const std::string &old_name,
...
bool Graph::removeEdge(const std::string &uname, const std::string &vname) {
    ...
    for (; it != m_edges.end(); ++it) {
        if (isUndirected()) {
            if (((*it)->first()->name() == uname &&
                     (*it)->second()->name() == vname) ||
                    ((*it)->first()->name() == vname &&
                     (*it)->second()->name() == uname)) {
                m_edges.erase(it);
                (*it)->first()->decDeg();
                (*it)->second()->decDeg();
